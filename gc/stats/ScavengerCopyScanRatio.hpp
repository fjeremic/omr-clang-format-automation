...
private:
	volatile uint64_t
	        _accumulatingSamples; /**< accumulator for aggregating per thread wait/copy/s`wait/copy/scanes-- these are periodically latched into _accumulatedSamples and reset */
	volatile uint64_t
	        _accumulatedSamples; /**< most recent aggregate wait/copy/scan counts from SCAVENGER_THREAD_UPDATES_PER_MAJOR_UPDATE samples  */
	volatile uintptr_t
	        _majorUpdateThreadEnv; /**< a token for the thread that has claimed a major update and owns the critical region wherein the update is effected */
	uintptr_t
	        _scalingUpdateCount; /**< the number of times _accumulatingSamples was latched into _accumulatedSamples */
		...
		if (SCAVENGER_SLOTS_SCANNED_PER_THREAD_UPDATE <= *slotsScanned) {
			...
			*slotsScanned = *slotsCopied = 0;
			...
			if (SCAVENGER_THREAD_UPDATES_PER_MAJOR_UPDATE == updateCount) {
				...
				if (0
				    == MM_AtomicOperations::lockCompareExchange(&_majorUpdateThreadEnv, 0,
				                                                (uintptr_t)env)) {
					return updateResult;
				}
			}
