...
public:
	struct FlipHistory
	{
		...
		uintptr_t _flipBytes[OBJECT_HEADER_AGE_MAX
		                     + 2]; /**< The historical number of bytes flipped in each age group */
		uintptr_t _tenureBytes[OBJECT_HEADER_AGE_MAX
		                       + 2]; /**< The historical number of bytes tenured in each age group */
	};
	...
	uintptr_t
	        _acquireListLockCount; /**< cumulative (for scan&free list) lock count. if this number is much larger than cumulative acquire list count, it indicates over-splitting */
	...
	uintptr_t
	        _totalDeepStructures; /**<  The number of deep structures that are scanned with priority (number of deepScanOutline function calls) */
	uintptr_t
	        _totalObjsDeepScanned; /**< The total number of deep structure objects that are special treated (number of copyAndForward with priority)*/
	...
	uintptr_t
	        _semiSpaceAllocBytesAcumulation; /**< Bytes allocated in new space between scavenges, updated on global collects. */
	uintptr_t
	        _tenureSpaceAllocBytesAcumulation; /**< Bytes allocated in tenure space between scavenges, updated on global collects. */
	...
	uint64_t
	        _readObjectBarrierUpdate; /**< Number of reference slots updates, which may be (often is) preceded by object copy */
...
public:
	uintptr_t
	        _flipHistoryNewIndex; /**< Index in to the first dimension of _flipHistory for the freshest history. */
