...
public:
	...
	uintptr_t
	        _bytesCardClean; /**< Bytes scanned through card cleaning (either for copy, or re-scanned for abort) */
	...
	uintptr_t
	        _nonEvacuateRegionCount; /**< Counts the number of regions which are not scheduled to be evacuated in collection set(mark/compact regions) - unused in per-thread stats)*/
	uintptr_t
	        _edenEvacuateRegionCount; /**< Counts the number of Eden regions selected to be evacuated in this copy-forward (only counted globally - unused in per-thread stats) */
	uintptr_t
	        _nonEdenEvacuateRegionCount; /**< Counts the number of non-Eden selected to be evacuated in this copy-forward (only counted globally - unused in per-thread stats) */
	uintptr_t
	        _edenSurvivorRegionCount; /**< Counts the number of Eden regions allocated for survivor space in this copy-forward (only counted globally - unused in per-thread stats) */
	uintptr_t
	        _nonEdenSurvivorRegionCount; /**< Counts the number of non-Eden regions allocated (that is, not tail-fill regions) for survivor space in this copy-forward (only counted globally - unused in per-thread stats) */
	...
	uintptr_t
	        _irrsStallCount; /**< The number of times the thread stalled to sync for InterRegionRememberedSet work */
	uintptr_t _workStallCount; /**< The number of times the thread stalled, and subsequently received more work */
	uintptr_t
	        _completeStallCount; /**< The number of times the thread stalled, and waited for all other threads to complete working */
	...
	uint64_t
	        _workStallTime; /**< The time, in hi-res ticks, the thread spent stalled waiting to receive more work */
	uint64_t
	        _completeStallTime; /**< The time, in hi-res ticks, the thread spent stalled waiting for all other threads to complete working */
	uintptr_t
	        _copiedArraysSplit; /**< The number of array chunks (not counting parts smaller than the split size) processed by this thread in copy-forward mode*/
	uintptr_t
	        _markedArraysSplit; /**< The number of array chunks (not counting parts smaller than the split size) processed by this thread in mark mode*/
#endif /* J9MODRON_TGC_PARALLEL_STATISTICS */

	uintptr_t _heapExpandedBytes; /**< Bytes by which the heap expanded in order to complete the collection */
	uintptr_t
	        _heapExpandedCount; /**< The number of times the heap was expanded in order to complete the collection */
	uint64_t
	        _heapExpandedTime; /**< Time taken expanding the heap in order to complete the collection, in hi-res ticks */

	uintptr_t
	        _externalCompactBytes; /**< The number of bytes selected for compaction outside of the copy-forward scheme. These would have been included in the copy-forward if there were sufficient space */

	uintptr_t
	        _objectsScannedFromWorkPackets; /**< In depth-first copy-forward, the number of objects scanned by this thread which it found on a work packet */
	uintptr_t
	        _objectsScannedFromOverflowedRegion; /**< In depth-first copy-forward, the number of objects scanned by this thread which it found in an overflowed region (while handling work packet overflow) */
	uintptr_t
	        _objectsScannedFromNextInChain; /**< In depth-first copy-forward, the number of objects scanned by this thread which it found as the next child to scan from an object it was already scanning (going "down" the tree) */
	uintptr_t
	        _objectsScannedFromDepthStack; /**< In depth-first copy-forward, the number of objects scanned by this thread which it found on its depth stack (that is, one it resumed as it was going "up" the tree) */
	uintptr_t
	        _objectsScannedFromRoot; /**< In depth-first copy-forward, the number of objects scanned by this thread which it found in the non-card root set */
