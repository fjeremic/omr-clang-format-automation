...
public:
	...
	uint64_t
	        _leafTaggingStartTime; /**< hires start time to tag arraylet leaf regions for fixup (one thread only) */
	uint64_t _leafTaggingEndTime; /**< hires end time to tag arraylet leaf regions for fixup (one thread only) */
	uint64_t
	        _regionCompactDataInitStartTime; /**< hires start time to initialize compact data in the region table (one thread only) */
	uint64_t
	        _regionCompactDataInitEndTime; /**< hires end time to initialize compact data in the region table (one thread only) */
	uint64_t
	        _clearMarkMapStartTime; /**< hires start time to clear the next mark map to store data for the compact set */
	uint64_t
	        _clearMarkMapEndTime; /**< hires end time to clear the next mark map to store data for the compact set */
	uint64_t
	        _rememberedSetClearingStartTime; /**< hires start time to clear all RSCL from-references for the regions in the compact set */
	uint64_t
	        _rememberedSetClearingEndTime; /**< hires end time to clear all RSCL from-references for the regions in the compact set */
	...
	uint64_t
	        _reportMoveEventsStartTime; /**< hires start time to report all object move events (master thread only and only if OBJECT_RENAME is hooked) */
	uint64_t
	        _reportMoveEventsEndTime; /**< hires end time to report all object move events (master thread only and only if OBJECT_RENAME is hooked) */
	uint64_t
	        _fixupExternalPacketsStartTime; /**< hires start time to fixup work packets for an active GMP (only done if GMP is active) */
	uint64_t
	        _fixupExternalPacketsEndTime; /**< hires end time to fixup work packets for an active GMP (only done if GMP is active) */
	uint64_t
	        _fixupArrayletLeafStartTime; /**< hires start time to fixup arraylet leaf spine pointers (one thread only) */
	uint64_t
	        _fixupArrayletLeafEndTime; /**< hires end time to fixup arraylet leaf spine pointers (one thread only) */
	uint64_t
	        _recycleStartTime; /**< hires start time to identify and recycle fully evacuated regions (one thread only) */
	uint64_t
	        _recycleEndTime; /**< hires end time to identify and recycle fully evacuated regions (one thread only) */
	...
	uint64_t
	        _finalClearNextMarkMapEndTime; /**< hires end time to clear the next mark map after compact is finished */
	uint64_t
	        _rebuildNextMarkMapStartTime; /**< hires start time to rebuild the next mark map from the next work packets (only done in GMP is active) */
	uint64_t
	        _rebuildNextMarkMapEndTime; /**< hires end time to rebuild the next mark map from the next work packets (only done in GMP is active) */
