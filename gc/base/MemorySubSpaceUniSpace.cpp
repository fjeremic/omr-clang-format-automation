...
MM_MemorySubSpaceUniSpace::performContract(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
	...
	*/
	...
	if (targetContractSize > maximumContractSize) {
		...
		Trc_MM_MemorySubSpaceUniSpace_performContract_Event1(env->getLanguageVMThread(), targetContractSize,
		                                                     maximumContractSize, contractSize);
	} else {
		contractSize = targetContractSize;
		Trc_MM_MemorySubSpaceUniSpace_performContract_Event2(env->getLanguageVMThread(), targetContractSize,
		                                                     maximumContractSize, contractSize);
	}
 ...
 */
bool
MM_MemorySubSpaceUniSpace::timeForHeapContract(MM_EnvironmentBase* env,
                                               MM_AllocateDescription* allocDescription,
                                               bool systemGC)
{
	Trc_MM_MemorySubSpaceUniSpace_timeForHeapContract_Entry(env->getLanguageVMThread(),
	                                                        systemGC ? "true" : "false");
	...
	if (systemGC) {
		...
		if (freeBytesAtSystemGCStart < minimumFree) {
			Trc_MM_MemorySubSpaceUniSpace_timeForHeapContract_Exit6(env->getLanguageVMThread(),
			                                                        freeBytesAtSystemGCStart, minimumFree);
			_contractionSize = 0;
			return false;
		}
	}
...
{
	Trc_MM_MemorySubSpaceUniSpace_calculateTargetContractSize_Entry(env->getLanguageVMThread(), allocSize,
	                                                                ratioContract ? "true" : "false");
	...
	if (allocSize > getApproximateActiveFreeMemorySize()) {
		...
		if (currentFree > maximumFree) {
			...
			uintptr_t targetHeapSize = ((currentHeapSize - currentFree)
...
MM_MemorySubSpaceUniSpace::calculateExpandSize(MM_EnvironmentBase* env, uintptr_t bytesRequired, bool expandToSatisfy)
{
	...
	Trc_MM_MemorySubSpaceUniSpace_calculateExpandSize_Exit1(env->getLanguageVMThread(), desiredFree, currentFree,
	                                                        expandSize);
	return expandSize;
}
...
uintptr_t
MM_MemorySubSpaceUniSpace::calculateCollectorExpandSize(MM_EnvironmentBase* env,
                                                        MM_Collector* requestCollector,
                                                        MM_AllocateDescription* allocDescription)
{
	uintptr_t expandSize;

	Trc_MM_MemorySubSpaceUniSpace_calculateCollectorExpandSize_Entry(env->getLanguageVMThread(),
...
MM_MemorySubSpaceUniSpace::checkForRatioExpand(MM_EnvironmentBase* env, uintptr_t bytesRequired)
{
	...
	if (gcPercentage < _extensions->heapExpansionGCTimeThreshold) {
		...
		Trc_MM_MemorySubSpaceUniSpace_checkForRatioExpand_Exit3(env->getLanguageVMThread(), gcPercentage,
		                                                        ratioExpandAmount);
 ...
 */
MMINLINE uintptr_t
MM_MemorySubSpaceUniSpace::adjustExpansionWithinSoftMax(MM_EnvironmentBase* env,
                                                        uintptr_t expandSize,
