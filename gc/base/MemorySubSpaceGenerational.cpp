...
void*
MM_MemorySubSpaceGenerational::allocateObject(MM_EnvironmentBase* env,
                                              ...
                                              bool shouldCollectOnFailure)
{
	...
	if (shouldCollectOnFailure) {
		...
		Trc_MM_MSSGenerational_allocate_exit(env->getLanguageVMThread(), "Object",
		                                     allocDescription->getBytesRequested(), 1, NULL);
		return NULL;
	} else {
		if (previousSubSpace == _memorySubSpaceNew) {
			...
			Trc_MM_MSSGenerational_allocate(env->getLanguageVMThread(), "Object",
			                                allocDescription->getBytesRequested(), _memorySubSpaceNew,
			                                _memorySubSpaceOld);
			void* result = _memorySubSpaceOld->allocateObject(env, allocDescription, baseSubSpace, this,
		...
		Trc_MM_MSSGenerational_allocate_exit(env->getLanguageVMThread(), "Object",
		                                     allocDescription->getBytesRequested(), 3, NULL);
                                                       ...
                                                       MM_MemorySubSpace* previousSubSpace)
{
	...
	Trc_MM_MSSGenerational_allocationRequestFailed_entry(env->getLanguageVMThread(),
	                                                     allocateDescription->getBytesRequested(), this, getName(),
	                                                     baseSubSpace, previousSubSpace, (uintptr_t)allocationType);
	...
	if (previousSubSpace == _memorySubSpaceNew) {
		...
		addr = _memorySubSpaceOld->allocationRequestFailed(env, allocateDescription, allocationType,
	...
	if (!env->acquireExclusiveVMAccessForGC(_collector, true, true)) {
		...
		Trc_MM_MSSGenerational_allocationRequestFailed(env->getLanguageVMThread(),
		                                               allocateDescription->getBytesRequested(), 2);
	...
	if (NULL != addr) {
		reportAllocationFailureEnd(env);
		Trc_MM_MSSGenerational_allocationRequestFailed_exit(env->getLanguageVMThread(),
		                                                    allocateDescription->getBytesRequested(), 5, addr);
		return addr;
	}
...
void*
MM_MemorySubSpaceGenerational::allocateArrayletLeaf(MM_EnvironmentBase* env,
                                                    MM_AllocateDescription* allocDescription,
                                                    MM_MemorySubSpace* baseSubSpace,
                                                    MM_MemorySubSpace* previousSubSpace,
                                                    bool shouldCollectOnFailure)
{
	if (shouldCollectOnFailure) {
		...
		if (previousSubSpace == _memorySubSpaceNew) {
			/* The allocate request is coming from new space - forward on to the old area */
			return _memorySubSpaceOld->allocateArrayletLeaf(env, allocDescription, baseSubSpace, this,
			                                                shouldCollectOnFailure);
		}
...
void*
MM_MemorySubSpaceGenerational::allocateTLH(MM_EnvironmentBase* env,
                                           MM_AllocateDescription* allocDescription,
                                           MM_ObjectAllocationInterface* objectAllocationInterface,
                                           MM_MemorySubSpace* baseSubSpace,
                                           MM_MemorySubSpace* previousSubSpace,
                                           bool shouldCollectOnFailure)
{
	if (shouldCollectOnFailure) {
		...
		if (previousSubSpace == _memorySubSpaceNew) {
			/* The allocate request is coming from new space - forward on to the old area */
			return _memorySubSpaceOld->allocateTLH(env, allocDescription, objectAllocationInterface,
                                           ...
                                           uintptr_t maximumSize)
{
	...
	if (memorySubSpace) {
		new (memorySubSpace) MM_MemorySubSpaceGenerational(
		        env, memorySubSpaceNew, memorySubSpaceOld, usesGlobalCollector, minimumSize, minimumSizeNew,
		        initialSizeNew, maximumSizeNew, minimumSizeOld, initialSizeOld, maximumSizeOld, maximumSize);
...
void
MM_MemorySubSpaceGenerational::checkResize(MM_EnvironmentBase* env,
                                           MM_AllocateDescription* allocDescription,
                                           bool _systemGC)
{
 ...
 */
uintptr_t
MM_MemorySubSpaceGenerational::getAvailableContractionSize(MM_EnvironmentBase* env,
                                                           MM_AllocateDescription* allocDescription)
{
 ...
 */
uintptr_t
MM_MemorySubSpaceGenerational::counterBalanceContract(MM_EnvironmentBase* env,
                                                      MM_MemorySubSpace* previousSubSpace,
                                                      ...
                                                      uintptr_t contractAlignment)
{
	...
	if (previousSubSpace == _memorySubSpaceNew) {
		return _memorySubSpaceOld->counterBalanceContractWithExpand(env, this, contractSubSpace, contractSize,
		                                                            contractAlignment, expandSize);
	}

	return _memorySubSpaceNew->counterBalanceContractWithExpand(env, this, contractSubSpace, contractSize,
	                                                            contractAlignment, expandSize);
}
