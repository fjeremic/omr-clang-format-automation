...
protected:
	...
	uintptr_t
	        _objectAlignmentShift; /**< cached copy of heap object alignment shift, must be log2(_objectAlignmentInBytes)  */
...
public:
	 ...
	 */
	MMINLINE omrobjectptr_t initializeAllocation(MM_EnvironmentBase* env,
	                                             void* allocatedBytes,
	 ...
	 */
	MMINLINE void calculateObjectDetailsForCopy(MM_EnvironmentBase* env,
			...
			do {
				...
				if (newFlags == oldFlags) {
					...
					break;
			         ...
			         != MM_AtomicOperations::lockCompareExchange(flagsPtr, (uintptr_t)oldFlags,
			                                                     (uintptr_t)newFlags));
			/* check to verify that this thread and no other thread atomically set flags -> toState */
			result = result && (0 == (oldFlags & rememberedMask));
		}
