...
public:
	enum RegionType
	{
		...
		ADDRESS_ORDERED_IDLE =
		        6, /**< Represents regions that have an AOL MemoryPool but are not actively being used (that is, they can be made into other kinds of regions without penalty) */
		ADDRESS_ORDERED_MARKED =
		        7, /**< Represents regions that have an AOL MemoryPool and an accurate mark set in extensions->previousMarkMap */
		BUMP_ALLOCATED = 8, /**< Represents regions that have a BumpPointer MemoryPool */
		BUMP_ALLOCATED_IDLE =
		        9, /**< Represents regions that have a BumpPointer MemoryPool but are not actively being used (that is, they can be made into other kinds of regions without penalty) */
		BUMP_ALLOCATED_MARKED =
		        10, /**< Represents regions that have a BumpPointer MemoryPool and an accurate mark set in extensions->previousMarkMap */
		LAST_REGION_TYPE /**< This dummy type is sed later as a size of an array for stats of region counts */
	};
	uintptr_t
	        _regionsInSpan; /**< most likely a temporary piece of data which, along with _nextInSet, builds a freelist (public for assertions) */
...
private:
	...
	MM_HeapRegionDescriptor*
	        _previousRegion; /**< previous region descriptor in the list (used by HeapRegionManager) */
	...
	MM_HeapRegionDescriptor*
	        _nextInSet; /**< most likely a temporary link which is used for passing around ad-hoc "collections" of regions */
...
public:
	...
	static bool initializer(MM_EnvironmentBase* env,
	                        ...
	                        void* highAddress);
	static void
	destructor(MM_EnvironmentBase* env, MM_HeapRegionManager* regionManager, MM_HeapRegionDescriptor* descriptor);
