...
private:
	MM_HeapRegionManager*
	        _heapRegionManager; /**< The Region Manager which contains all the descriptors used for allocation (they contain the Memory Pools which manage their underlying memory) */
	...
	MM_FreeHeapRegionList*
	        _coalesceFreeList; /**< Free regions that might be coalescable, so avoid allocating from them if at all possible. */
	...
	*/
	MM_LockingHeapRegionQueue* _smallAvailableRegions
	        ...
	        [NUM_DEFRAG_BUCKETS]; /**< Regions that are available to be given out to allocation contexts and aren't entirely free. */
	...
	*/
	MM_HeapRegionQueue*
	        _arrayletAvailableRegions; /**< Arraylet regions that are available to be given out to allocation contexts and aren't entirely free. */
	...
	uintptr_t
	        _splitAvailableListSplitCount; /* number of split available region queues per size class per defragment bucket */
	uint8_t _skipAvailableRegionForAllocation
	        [OMR_SIZECLASSES_NUM_SMALL
	         + 1]; /* per size class flag to indicate if there is any available regions left for allocation for that size class */
...
public:
	...
	static MM_HeapRegionQueue* allocateHeapRegionQueue(MM_EnvironmentBase* env,
	                                                   ...
	                                                   bool trackFreeBytes);
	static MM_FreeHeapRegionList* allocateFreeHeapRegionList(MM_EnvironmentBase* env,
	                                                         MM_HeapRegionList::RegionListKind regionListKind,
	                                                         bool singleRegionsOnly);
	MM_HeapRegionDescriptorSegregated*
	allocateFromRegionPool(MM_EnvironmentBase* env, uintptr_t numRegions, uintptr_t szClass, uintptr_t maxExcess);
	MM_HeapRegionDescriptorSegregated*
	allocateRegionFromSmallSizeClass(MM_EnvironmentBase* env, uintptr_t sizeClass);
	MM_HeapRegionDescriptorSegregated* allocateRegionFromArrayletSizeClass(MM_EnvironmentBase* env);
	MM_HeapRegionDescriptorSegregated*
	sweepAndAllocateRegionFromSmallSizeClass(MM_EnvironmentBase* env, uintptr_t sizeClass);
	...
	void
	addFreeRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptorSegregated* region, bool alreadyFree = false);
	...
	getSmallAvailableRegions(uintptr_t sizeClass, uintptr_t defragBucket, uintptr_t splitList)
	{
