...
MM_RegionPoolSegregated::initialize(MM_EnvironmentBase* env)
{
	...
	for (szClass = OMR_SIZECLASSES_MIN_SMALL; szClass <= OMR_SIZECLASSES_MAX_SMALL; szClass++) {
		for (int32_t i = 0; i < NUM_DEFRAG_BUCKETS; i++) {
			...
			for (uintptr_t j = 0; j < _splitAvailableListSplitCount; j++) {
				...
				new (&regionQueue[j]) MM_LockingHeapRegionQueue(MM_HeapRegionList::HRL_KIND_AVAILABLE,
				                                                true, true, true);
	...
	_arrayletFullRegions = MM_RegionPoolSegregated::allocateHeapRegionQueue(env, MM_HeapRegionList::HRL_KIND_FULL,
	                                                                        true, true, false);
	_arrayletSweepRegions = MM_RegionPoolSegregated::allocateHeapRegionQueue(env, MM_HeapRegionList::HRL_KIND_SWEEP,
	                                                                         true, true, false);
	if ((NULL == _arrayletAvailableRegions) || (NULL == _arrayletFullRegions) || (NULL == _arrayletSweepRegions)) {
		return false;
	}

	_largeFullRegions = MM_RegionPoolSegregated::allocateHeapRegionQueue(env, MM_HeapRegionList::HRL_KIND_FULL,
	                                                                     false, true, false);
	_largeSweepRegions = MM_RegionPoolSegregated::allocateHeapRegionQueue(env, MM_HeapRegionList::HRL_KIND_SWEEP,
	                                                                      false, true, false);
...
MM_RegionPoolSegregated::allocateHeapRegionQueue(MM_EnvironmentBase* env,
                                                 ...
                                                 bool trackFreeBytes)
{
	return MM_LockingHeapRegionQueue::newInstance(env, regionListKind, singleRegionsOnly, concurrentAccess,
	                                              trackFreeBytes);
}

MM_FreeHeapRegionList*
MM_RegionPoolSegregated::allocateFreeHeapRegionList(MM_EnvironmentBase* env,
                                                    MM_HeapRegionList::RegionListKind regionListKind,
                                                    bool singleRegionsOnly)
{
...
void
MM_RegionPoolSegregated::enqueueAvailable(MM_HeapRegionDescriptorSegregated* region,
                                          uintptr_t sizeClass,
                                          uintptr_t occupancy,
...
void
MM_RegionPoolSegregated::addFreeRegion(MM_EnvironmentBase* env,
