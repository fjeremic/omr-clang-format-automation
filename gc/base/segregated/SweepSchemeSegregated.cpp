...
MM_SweepSchemeSegregated::unmarkRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptorSegregated* region)
{
	...
	if (lastCellStart + scanBitOffset >= regionHighAddress) {
		...
		_markMap->getSlotIndexAndMask((omrobjectptr_t)(lastCellStart + scanBitOffset), &slotIndexHigh,
		                              &bitMask);
	}
	...
	while (regionPool->getCurrentTotalCountOfSweepRegions()) {
		for (uintptr_t sizeClass = OMR_SIZECLASSES_MIN_SMALL; sizeClass <= OMR_SIZECLASSES_MAX_SMALL;
			...
			while (regionPool->getCurrentCountOfSweepRegions(sizeClass)) {
				...
				if ((actualSweepRegions = sweepList->dequeue(env->getRegionWorkList(),
				                                             sweepSmallRegionsPerIteration))
				    > 0) {
