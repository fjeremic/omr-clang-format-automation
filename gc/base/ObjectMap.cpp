...
MM_ObjectMap::newInstance(MM_EnvironmentBase* env)
{
	...
	objectMap = (MM_ObjectMap*)env->getForge()->allocate(sizeof(MM_ObjectMap), OMR::GC::AllocationCategory::FIXED,
	                                                     OMR_GET_CALLSITE());
	if (objectMap) {
...
MM_ObjectMap::markValidObjectForRange(MM_EnvironmentBase* env, void* heapBase, void* heapTop)
{
	...
	GC_ObjectHeapIteratorAddressOrderedList objectHeapIterator(env->getExtensions(), (omrobjectptr_t)heapBase,
	                                                           heapTop, false, false);
