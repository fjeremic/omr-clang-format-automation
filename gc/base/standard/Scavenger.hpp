...
private:
	...
	volatile uintptr_t
	        _doneIndex; /**< sequence ID of completeScan loop, which we may have a few during one GC cycle */
	...
	uintptr_t
	        _waitingCountAliasThreshold; /**< Only alias a copy cache IF the number of threads waiting hasn't reached the threshold*/
	volatile uintptr_t
	...
	volatile bool
	        _shouldYield; /**< Set by the first GC thread that observes that a criteria for yielding is met. Reset only when the concurrent phase is finished. */
...
public:
	 ...
	 */
	static void
	hookGlobalCollectionStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData);

	/**
	 * Hook callback. Called when a global collect has completed
	 */
	static void
	hookGlobalCollectionComplete(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData);
	...
	uintptr_t calculateCopyScanCacheSizeForWaitingThreads(uintptr_t maxCacheSize,
	...
	MMINLINE bool scavengeObjectSlots(MM_EnvironmentStandard* env,
	                                  ...
	                                  uintptr_t flags,
	                                  omrobjectptr_t* rememberedSetSlot);
	MMINLINE MM_CopyScanCacheStandard* incrementalScavengeObjectSlots(MM_EnvironmentStandard* env,
	                                                                  omrobjectptr_t objectPtr,
	                                                                  MM_CopyScanCacheStandard* scanCache);
	...
	MMINLINE MM_CopyScanCacheStandard* aliasToCopyCache(MM_EnvironmentStandard* env,
	...
	MMINLINE void
	addCacheEntryToScanListAndNotify(MM_EnvironmentStandard* env, MM_CopyScanCacheStandard* newCacheEntry);
	...
	virtual void masterThreadGarbageCollect(MM_EnvironmentBase* env,
	                                        MM_AllocateDescription* allocDescription,
...
public:
	...
	virtual void postConcurrentUpdateStatsAndReport(MM_EnvironmentBase* env,
	 ...
	 */
	void
	triggerConcurrentScavengerTransition(MM_EnvironmentBase* envBase, MM_AllocateDescription* allocDescription);
	/**
