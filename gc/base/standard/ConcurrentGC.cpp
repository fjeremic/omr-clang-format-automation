...
con_helper_thread_proc2(OMRPortLibrary* portLib, void* info)
{
	...
	OMR_VMThread* omrThread = MM_EnvironmentBase::attachVMThread(omrVM, "Concurrent Mark Helper",
...
MM_ConcurrentGC::reportConcurrentKickoff(MM_EnvironmentBase* env)
{
	...
	TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_KICKOFF(
	        ...
	        J9HOOK_MM_PRIVATE_CONCURRENT_KICKOFF, _extensions->getHeap()->initializeCommonGCData(env, &commonData),
	        _stats.getTraceSizeTarget(), _stats.getKickoffThreshold(), _stats.getRemainingFree(),
	        _stats.getKickoffReason(), _languageKickoffReason);
}
...
MM_ConcurrentGC::reportConcurrentAborted(MM_EnvironmentBase* env, CollectionAbortReason reason)
{
	...
	TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_ABORTED(_extensions->privateHookInterface, env->getOmrVMThread(),
	                                             omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_ABORTED,
	                                             reason);
}
...
MM_ConcurrentGC::reportConcurrentHalted(MM_EnvironmentBase* env)
{
	...
	Trc_MM_ConcurrentHalted(
	        env->getLanguageVMThread(), (uintptr_t)_stats.getExecutionModeAtGC(), _stats.getTraceSizeTarget(),
	        _stats.getTotalTraced(), _stats.getMutatorsTraced(), _stats.getConHelperTraced(),
	...
	TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_HALTED(
	        _extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
	        J9HOOK_MM_PRIVATE_CONCURRENT_HALTED, (uintptr_t)_stats.getExecutionModeAtGC(),
...
MM_ConcurrentGC::reportConcurrentCollectionStart(MM_EnvironmentBase* env)
{
	...
	Trc_MM_ConcurrentCollectionStart(
	        env->getLanguageVMThread(), _extensions->heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_NEW),
	...
	Trc_MM_ExclusiveAccess(
	        env->getLanguageVMThread(), (uint32_t)(exclusiveAccessTimeMicros / 1000),
	...
	if (J9_EVENT_IS_HOOKED(_extensions->privateHookInterface, J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START)) {
		...
		ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START(
		        _extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
		        J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START, &commonData, _stats.getTraceSizeTarget(),
		        _stats.getTotalTraced(), _stats.getMutatorsTraced(), _stats.getConHelperTraced(),
		        cardTable->getCardTableStats()->getConcurrentCleanedCards(), _stats.getCardCleaningThreshold(),
...
MM_ConcurrentGC::reportConcurrentCollectionEnd(MM_EnvironmentBase* env, uint64_t duration)
{
	...
	if (J9_EVENT_IS_HOOKED(_extensions->privateHookInterface, J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END)) {
		...
		ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END(
		        _extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
		        J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END, duration, env->getExclusiveAccessTime(),
		        &commonData);
	}
}
...
MM_ConcurrentGC::reportConcurrentBackgroundThreadActivated(MM_EnvironmentBase* env)
{
	...
	TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_BACKGROUND_THREAD_ACTIVATED(_extensions->privateHookInterface,
	                                                                 env->getOmrVMThread());
}
...
MM_ConcurrentGC::reportConcurrentBackgroundThreadFinished(MM_EnvironmentBase* env, uintptr_t traceTotal)
{
	...
	TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_BACKGROUND_THREAD_FINISHED(_extensions->privateHookInterface,
	                                                                env->getOmrVMThread(), traceTotal);
}
...
MM_ConcurrentGC::reportConcurrentCompleteTracingStart(MM_EnvironmentBase* env)
{
	...
	TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COMPLETE_TRACING_START(
	        _extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
	        J9HOOK_MM_PRIVATE_CONCURRENT_COMPLETE_TRACING_START, _stats.getConcurrentWorkStackOverflowCount());
}
...
MM_ConcurrentGC::reportConcurrentCompleteTracingEnd(MM_EnvironmentBase* env, uint64_t duration)
{
	...
	TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COMPLETE_TRACING_END(
	        _extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
	        J9HOOK_MM_PRIVATE_CONCURRENT_COMPLETE_TRACING_END, duration, _stats.getCompleteTracingCount(),
	        _stats.getConcurrentWorkStackOverflowCount());
}
...
MM_ConcurrentGC::reportConcurrentRememberedSetScanStart(MM_EnvironmentBase* env)
{
	...
	TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_START(
	        _extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
	        J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_START, _stats.getConcurrentWorkStackOverflowCount());
}
...
MM_ConcurrentGC::reportConcurrentRememberedSetScanEnd(MM_EnvironmentBase* env, uint64_t duration)
{
	...
	TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_END(
	        _extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
	        J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_END, duration, _stats.getRSObjectsFound(),
...
MM_ConcurrentGC::initialize(MM_EnvironmentBase* env)
{
	...
	if (omrthread_monitor_init_with_name(&_conHelpersActivationMonitor, 0,
	...
	if (_secondCardCleanPass) {
		_cardCleaningFactorPass2 =
		        interpolateInRange(INITIAL_CARD_CLEANING_FACTOR_PASS2_1, INITIAL_CARD_CLEANING_FACTOR_PASS2_8,
		                           INITIAL_CARD_CLEANING_FACTOR_PASS2_10, _allocToTraceRateNormal);

		_maxCardCleaningFactorPass2 =
		        interpolateInRange(MAX_CARD_CLEANING_FACTOR_PASS2_1, MAX_CARD_CLEANING_FACTOR_PASS2_8,
	...
	_allocToTraceRateMaxFactor = interpolateInRange(MAX_ALLOC_2_TRACE_RATE_1, MAX_ALLOC_2_TRACE_RATE_8,
	...
	if (_extensions->debugConcurrentMark) {
		...
		omrtty_printf(
		        "                           Allocate to trace Rate Factors Minimum=\"%f\" Maximum=\"%f\"\n",
	...
	while (!initDone) {
		...
		while (NULL != (region = regionIterator.nextRegion())) {
			...
			if (_numInitRanges < _numPhysicalInitRanges) {
				...
				_initRanges[i].initBytes = _markingScheme->numMarkBitsInRange(env, _initRanges[i].base,
				                                                              _initRanges[i].top);
...
{
	MM_EnvironmentBase::detachVMThread(_extensions->getOmrVM(), omrThread,
	                                   MM_EnvironmentBase::ATTACH_GC_HELPER_THREAD);
	omrthread_monitor_enter(_conHelpersActivationMonitor);
...
MM_ConcurrentGC::initializeConcurrentHelpers(MM_GCExtensionsBase* extensions)
{
	...
	for (conHelperThreadCount = 0; conHelperThreadCount < _conHelperThreads; conHelperThreadCount++) {
		...
		threadForkResult = createThreadWithCategory(
		        &(_conHelpersTable[conHelperThreadCount]), OMR_OS_STACK_SIZE, J9THREAD_PRIORITY_MIN, 0,
		        con_helper_thread_proc, (void*)&conHelperThreadInfo, J9THREAD_CATEGORY_SYSTEM_GC_THREAD);
...
MM_ConcurrentGC::tuneToHeap(MM_EnvironmentBase* env)
{
	...
	kickoffThresholdPlusBuffer = (uintptr_t)((float)kickoffThreshold + boost
	                                         + ((float)_extensions->concurrentSlack * kickoffProportion));
	_stats.setKickoffThreshold(kickoffThresholdPlusBuffer);
	_stats.setCardCleaningThreshold((uintptr_t)((float)cardCleaningThreshold + boost
...
MM_ConcurrentGC::updateTuningStatistics(MM_EnvironmentBase* env)
{
	...
	if (env->_cycleState->_gcCode.isExplicitGC() || (CONCURRENT_TRACE_ONLY > _stats.getExecutionModeAtGC())) {
		...
		_tenureLiveObjectFactor = MM_Math::weightedAverage(_tenureLiveObjectFactor, newLiveObjectFactor,
		...
		*/
		...
		_tenureNonLeafObjectFactor = MM_Math::weightedAverage(_tenureNonLeafObjectFactor,
		                                                      newNonLeafObjectFactor, NON_LEAF_HISTORY_WEIGHT);
...
MM_ConcurrentGC::potentialFreeSpace(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
	...
	if (LOA == _meteringType) {
		...
		headRoom = (uintptr_t)(
		        _extensions->concurrentKickoffTenuringHeadroom
                                ...
                                MM_AllocateDescription* allocDescription)
{
	...
	while (!taxPaid) {
		...
		case CONCURRENT_CLEAN_TRACE:
			...
			if (sizeToTrace > 0) {
				sizeTraced = doConcurrentTrace(env, allocDescription, sizeToTrace, subspace,
				                               threadAtSafePoint);
			}
	...
	while (initToDo > initDone) {
		...
		if (getInitRange(env, &from, &to, &type, &concurrentCollectable)) {
			...
			case CARD_TABLE:
				if (NULL != _cardTable) {
					initDone +=
					        ((MM_ConcurrentCardTable*)_cardTable)->clearCardsInRange(env, from, to);
				}
				break;
			default: assume0(0);
			}
...
MM_ConcurrentGC::localMark(MM_EnvironmentBase* env, uintptr_t sizeToTrace)
{
	...
	while (NULL != (objectPtr = (omrobjectptr_t)env->_workStack.popNoWait(env))) {
		...
		if ((uintptr_t)objectPtr & PACKET_ARRAY_SPLIT_TAG) {
			...
			sizeTraced += _markingScheme->scanObject(env, objectPtr, SCAN_REASON_PACKET,
	...
	if ((J9MMCONSTANT_EXPLICIT_GC_IDLE_GC == gcCode) && (CONCURRENT_OFF < executionModeAtGC)) {
		...
		if (CONCURRENT_FINAL_COLLECTION > executionModeAtGC) {
			...
			if (!_markingScheme->getWorkPackets()->tracingExhausted()) {
				 ...
				 * final card cleaning.
				 */
				MM_ConcurrentCompleteTracingTask completeTracingTask(env, _dispatcher, this,
				                                                     env->_cycleState);
		...
		if (_extensions->scavengerEnabled) {
			...
			if (!(_extensions->rememberedSet.isEmpty())) {
				MM_ConcurrentScanRememberedSetTask scanRememberedSetTask(env, _dispatcher, this,
				                                                         env->_cycleState);
 ...
 */
bool
MM_ConcurrentGC::internalGarbageCollect(MM_EnvironmentBase* env,
                                        MM_MemorySubSpace* subSpace,
                                        MM_AllocateDescription* allocDescription)
{
	_extensions->globalGCStats.gcCount += 1;

	masterThreadGarbageCollect(env, static_cast<MM_AllocateDescription*>(allocDescription), _initializeMarkMap,
	                           false);
...
{
	Trc_MM_ConcurrentGC_heapRemoveRange_Entry(env->getLanguageVMThread(), subspace, size, lowAddress, highAddress,
	...
	bool result = MM_ParallelGlobalGC::heapRemoveRange(env, subspace, size, lowAddress, highAddress,
	                                                   lowValidAddress, highValidAddress);

	/* ...and then contract the card table */
	result = result
	        && ((MM_ConcurrentCardTable*)_cardTable)
	                   ->heapRemoveRange(env, subspace, size, lowAddress, highAddress, lowValidAddress,
	                                     highValidAddress);
				...
				if ((objectPtr >= _heapBase) && (objectPtr < _heapAlloc)
					...
					if (_extensions->dirtCardDuringRSScan) {
						...
						if (env->_workStack.getPushCount() >= maxPushes) {
							...
							while (NULL
							       != (objectPtr =
							                   (omrobjectptr_t)env->_workStack.popNoWait(
							                           env))) {
								bytesTraced += _markingScheme->scanObject(
