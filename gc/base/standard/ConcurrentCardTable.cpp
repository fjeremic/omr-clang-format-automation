...
MM_ConcurrentCardTable::tlhCleared(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
	...
	((MM_ConcurrentCardTable*)userData)
	        ->processTLHMarkBits(env, (MM_MemorySubSpace*)event->subSpace, event->cacheBase, event->cacheTop,
	                             CLEAR);
}
 ...
 */
bool
MM_ConcurrentCardTable::allocateCardTableEntriesForHeapRange(MM_EnvironmentBase* env,
 ...
 */
bool
MM_ConcurrentCardTable::freeCardTableEntriesForHeapRange(MM_EnvironmentBase* env,
                                                         uintptr_t size,
                                                         void* lowAddress,
                                                         void* highAddress,
 ...
 */
bool
MM_ConcurrentCardTable::allocateTLHMarkMapEntriesForHeapRange(MM_EnvironmentBase* env,
                                                              MM_MemorySubSpace* subspace,
                                                              uintptr_t size,
                                                              void* lowAddress,
                                                              void* highAddress)
{
	...
	if (_extensions->isFvtestForceConcurrentTLHMarkMapCommitFailure()) {
		...
		if (NULL != _tlhMarkBits) {
			...
			commited = memoryManager->commitMemory(&_tlhMarkMapMemoryHandle,
 ...
 */
bool
MM_ConcurrentCardTable::freeTLHMarkMapEntriesForHeapRange(MM_EnvironmentBase* env,
                                                          uintptr_t size,
                                                          void* lowAddress,
                                                          void* highAddress,
                                                          void* lowValidAddress,
                                        ...
                                        void* highValidAddress)
{
	...
	if (NULL != _heapAlloc) {
		...
		/* Adjust card table  to reflect heap change */
		result = freeCardTableEntriesForHeapRange(env, size, lowAddress, highAddress, lowValidAddress,
		                                          highValidAddress);
		...
		if (result) {
			if (subspace->isConcurrentCollectable()) {
				...
				result = freeTLHMarkMapEntriesForHeapRange(env, size, lowAddress, highAddress,
...
MM_ConcurrentCardTable::initialize(MM_EnvironmentBase* env, MM_Heap* heap)
{
	...
	if (initialized) {
...
#endif /* OMR_GC_MODRON_SCAVENGER */
		{
			...
			MM_MemoryManager* memoryManager = _extensions->memoryManager;
			if (!memoryManager->createVirtualMemoryForMetadata(env, &_tlhMarkMapMemoryHandle,
			                                                   sizeof(uintptr_t), tlhMarkMapSizeRequired)) {
				return false;
			}
...
MM_ConcurrentCardTable::initializeCardCleaning(MM_EnvironmentBase* env)
{
	...
	if (_extensions->cardCleaningPasses > 0) {
		MM_AtomicOperations::lockCompareExchangeU32((volatile uint32_t*)&_cardCleanPhase,
		                                            (uint32_t)_cardCleanPhase, (uint32_t)UNINITIALIZED);
	...
	while (NULL != firstCard) {
		...
		for (currentCard = firstCard; currentCard < lastCardToClean; currentCard++) {
			...
			if (((Card)CARD_CLEAN == *currentCard) && (0 == (uintptr_t)currentCard % sizeof(uintptr_t))) {
				 ...
				 **/
				uintptr_t* lastSlot = (uintptr_t*)MM_Math::roundToFloor(sizeof(uintptr_t),
				                                                        (uintptr_t)lastCardToClean);
