...
public:
	...
	/* New methods */
	void
	initializeFragment(MM_EnvironmentBase* env, MM_GCRememberedSetFragment* fragment); /* "Nulls" out a fragment. */
	void
	storeInFragment(MM_EnvironmentBase* env,
	...
	void
	flushFragments(MM_EnvironmentBase*
	                       env); /* Ensures all fragments will be seen as invalid next time they are accessed. */
...
private:
	void setGlobalIndex(MM_EnvironmentBase* env,
	                    UDATA indexValue); /* Increments the appropriate global index (global or preserved). */
};
