...
MM_ParallelGlobalGC::initialize(MM_EnvironmentBase* env)
{
	...
	(*mmPrivateHooks)
	        ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_SYSTEM_GC_END, globalGCHookSysEnd,
	                                     OMR_GET_CALLSITE(), NULL);
	...
	if (_extensions->scavengerEnabled) {
		...
		if (_extensions->isConcurrentScavengerEnabled()) {
			(*mmPrivateHooks)
			        ->J9HookRegisterWithCallSite(mmPrivateHooks, J9HOOK_MM_PRIVATE_SWEEP_END,
			                                     hookGlobalGcSweepEndAbortedCSFixHeap, OMR_GET_CALLSITE(),
			                                     this);
		}
#endif /* OMR_GC_CONCURRENT_SCAVENGER */
	}
...
void
MM_ParallelGlobalGC::masterThreadGarbageCollect(MM_EnvironmentBase* env,
...
#if defined(OMR_GC_MODRON_COMPACTION)
bool
MM_ParallelGlobalGC::shouldCompactThisCycle(MM_EnvironmentBase* env,
                                            MM_AllocateDescription* allocDescription,
                                            ...
                                            MM_GCCode gcCode)
{
	...
	if (_extensions->scavengerEnabled) {
		...
		if (failedTenureLargest > 0) {
			MM_MemorySpace* memorySpace = env->getMemorySpace();
			MM_AllocateDescription tenureAllocDescription(failedTenureLargest,
...
MM_ParallelGlobalGC::sweep(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription, bool rebuildMarkBits)
{
	...
	if (0 != activeSubSpace->getContractionSize()) {
		_compactThisCycle = compactRequiredBeforeHeapContraction(env, allocDescription,
		                                                         activeSubSpace->getContractionSize());
	}
#endif /* OMR_GC_MODRON_COMPACTION */
...
#if defined(OMR_GC_MODRON_COMPACTION)
void
MM_ParallelGlobalGC::masterThreadCompact(MM_EnvironmentBase* env,
                                         MM_AllocateDescription* allocDescription,
                                         bool rebuildMarkBits)
{
	OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
 ...
 */
void
MM_ParallelGlobalGC::internalPreCollect(MM_EnvironmentBase* env,
...
bool
MM_ParallelGlobalGC::internalGarbageCollect(MM_EnvironmentBase* env,
                                            MM_MemorySubSpace* subSpace,
                                            MM_AllocateDescription* allocDescription)
{
                                     ...
                                     void* highValidAddress)
{
	...
	result = result
	        && _delegate.heapRemoveRange(env, subspace, size, lowAddress, highAddress, lowValidAddress,
	                                     highValidAddress);
...
MM_ParallelGlobalGC::reportGCEnd(MM_EnvironmentBase* env)
{
	...
	TRIGGER_J9HOOK_MM_OMR_GLOBAL_GC_END(
	        _extensions->omrHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
