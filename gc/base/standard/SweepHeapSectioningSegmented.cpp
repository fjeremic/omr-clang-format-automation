...
MM_SweepHeapSectioningSegmented::estimateTotalChunkCount(MM_EnvironmentBase* env)
{
	...
	if (0 == _extensions->parSweepChunkSize) {
		...
		_extensions->parSweepChunkSize =
		        MM_Math::roundToCeiling(256 * 1024,
		...
		if (region->isCommitted()) {
			...
			while (heapChunkBase < regionHighAddress) {
				...
				if (((uintptr_t)regionHighAddress - (uintptr_t)heapChunkBase)
					...
					heapChunkTop =
					        (uintptr_t*)((uintptr_t)heapChunkBase + _extensions->parSweepChunkSize);
				}
				 ...
				 */
				pool = region->getSubSpace()->getMemoryPool(env, heapChunkBase, heapChunkTop,
				                                            poolHighAddr);
