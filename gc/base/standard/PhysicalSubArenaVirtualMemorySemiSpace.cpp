...
MM_PhysicalSubArenaVirtualMemorySemiSpace::expand(MM_EnvironmentBase* env, uintptr_t expandSize)
{
	...
	assume0(expandSize
	        == MM_Math::roundToCeiling(MM_GCExtensions::getExtensions(env)->heapAlignment,
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::contract(MM_EnvironmentBase* env, uintptr_t contractSize)
{
	...
	if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
		...
		getHeapRegionManager()->resizeAuxillaryRegion(env, _highSemiSpaceRegion, survivorSegmentBase,
		                                              survivorSegmentTop);
		getHeapRegionManager()->resizeAuxillaryRegion(env, _lowSemiSpaceRegion, allocateSegmentBase,
		                                              allocateSegmentTop);
		...
		if (allocateSegmentBase > allocateLeadingFreeTop) {
			...
			subSpaceAllocate->moveHeap(env, allocateLeadingFreeTop, allocateTrailingFreeBase,
			                           allocateSegmentBase);
		...
		if (allocateSegmentBase < allocateLeadingFreeTop) {
			...
			subSpaceAllocate->addExistingMemory(
			        env, this, ((uintptr_t)allocateLeadingFreeTop) - ((uintptr_t)allocateSegmentBase),
		...
		subSpaceAllocate->addExistingMemory(
		        env, this, (uintptr_t)(allocateSegmentTop - (uintptr_t)allocateSpaceUsedHeapTop),
		...
		/* we are subtly assuming that allocate size is at least as large as survivor size (the converse makes no sense) so ensure this assumption is correct */
		Assert_MM_true(allocateSizeConsumed >= survivorSizeConsumed);
		getHeapRegionManager()->resizeAuxillaryRegion(env, _highSemiSpaceRegion, allocateSegmentBase,
		                                              allocateSegmentTop);
		getHeapRegionManager()->resizeAuxillaryRegion(env, _lowSemiSpaceRegion, survivorSegmentBase,
		                                              survivorSegmentTop);
                                                ...
                                                bool updateMemoryPools)
{
	...
	if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
		...
		getHeapRegionManager()->resizeAuxillaryRegion(env, _highSemiSpaceRegion, highBase,
		                                              _highSemiSpaceRegion->getHighAddress());
		...
		getHeapRegionManager()->resizeAuxillaryRegion(env, _lowSemiSpaceRegion,
		                                              _lowSemiSpaceRegion->getLowAddress(), expandTop);
		...
		getHeapRegionManager()->resizeAuxillaryRegion(env, _lowSemiSpaceRegion,
		                                              _lowSemiSpaceRegion->getLowAddress(), lowTop);
		...
		/* Adjust the upper boundary of the allocate (low) segment */
		getHeapRegionManager()->resizeAuxillaryRegion(env, _highSemiSpaceRegion, lowTop,
		                                              _highSemiSpaceRegion->getHighAddress());
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::tilt(MM_EnvironmentBase* env, uintptr_t survivorSpaceSizeRequested)
{
	...
	minimumBytesFree = (uintptr_t)((_lowSemiSpaceRegion->getSize() + _highSemiSpaceRegion->getSize())
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::expandNoCheck(MM_EnvironmentBase* env, uintptr_t expandSize)
{
	...
	Assert_MM_true(_subSpace->getCurrentSize()
	...
	Assert_MM_true(splitExpandSize
	               == MM_Math::roundToCeiling(getHeapRegionManager()->getRegionSize(), splitExpandSize));
	...
	Assert_MM_true(allocateSpaceExpandSize
	               == MM_Math::roundToCeiling(extensions->heapAlignment, allocateSpaceExpandSize));
	Assert_MM_true(survivorSpaceExpandSize
	               == MM_Math::roundToCeiling(extensions->heapAlignment, survivorSpaceExpandSize));
	/* Find where the allocate subspace is */
	MM_MemorySubSpace* subSpaceAllocate = ((MM_MemorySubSpaceSemiSpace*)_subSpace)->getMemorySubSpaceAllocate();
	MM_MemorySubSpace* subSpaceSurvivor = ((MM_MemorySubSpaceSemiSpace*)_subSpace)->getMemorySubSpaceSurvivor();
	...
	if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
		...
		if (0 != allocateSpaceExpandSize) {
			...
			freeRangeToTransferBase = (void*)MM_Math::roundToCeiling(extensions->heapAlignment,
		...
		getHeapRegionManager()->resizeAuxillaryRegion(env, _lowSemiSpaceRegion, newLowAddress,
		                                              freeRangeToTransferBase);
		getHeapRegionManager()->resizeAuxillaryRegion(env, _highSemiSpaceRegion, freeRangeToTransferBase,
		                                              _highSemiSpaceRegion->getHighAddress());
 ...
 */
uintptr_t
MM_PhysicalSubArenaVirtualMemorySemiSpace::calculateExpansionSplit(MM_EnvironmentBase* env,
	...
	if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
		...
		if (freeRangeToTransferSize < survivorSpaceExpandSize) {
			...
			if (extensions->tiltedScavenge) {
				...
				expandSize = MM_Math::roundToCeiling(getHeapRegionManager()->getRegionSize() * 2,
				                                     expandSize);
 ...
 */
uintptr_t
MM_PhysicalSubArenaVirtualMemorySemiSpace::checkCounterBalanceExpand(MM_EnvironmentBase* env,
