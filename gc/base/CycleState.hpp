...
public:
	enum CollectionType
	{
		...
		CT_GLOBAL_MARK_PHASE =
		        2, /**< Global mark phase type (performs a full heap trace to build a fresh mark map) */
	};
	...
	volatile bool
	        _dynamicClassUnloadingEnabled; /**< Determine if dynamic class unloading is enabled for this cycle */
	...
	{
		uintptr_t
		        _survivorSetRegionCount; /**< Count of regions Copy Forward used as destination. Effective usage of regions is accounted, thus the count is not a cardinal number. */
	} _pgcData;

	uintptr_t
	        _currentIncrement; /**< The index of the current increment within the cycle, starting at 0 (used for event reporting) */
	...
	MM_CycleState*
	        _externalCycleState; /**< Another state which overlaps with this and needs to be fixed up to reflect any changes to the heap made under this cycle */
	uintptr_t _type;
	uintptr_t
	        _verboseContextID; /**< the verbose context id for this cycle.  This should only be read/written to by verbose gc */
	...
	enum ReferenceObjectOptions
	{
		...
		references_soft_as_weak =
		        0x08 /**< treat SoftReferences as weak (ignore the age) (set for OOM collections) */
	};

	uintptr_t
	        _referenceObjectOptions; /**< A bitmask controlling how Reference objects are treated during marking */
