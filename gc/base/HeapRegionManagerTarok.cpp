...
MM_HeapRegionManagerTarok::MM_HeapRegionManagerTarok(MM_EnvironmentBase* env,
...
MM_HeapRegionManagerTarok::newInstance(MM_EnvironmentBase* env,
                                       uintptr_t regionSize,
                                       uintptr_t tableDescriptorSize,
                                       MM_RegionDescriptorInitializer regionDescriptorInitializer,
                                       MM_RegionDescriptorDestructor regionDescriptorDestructor)
{
	...
	if (NULL != regionManager) {
		new (regionManager) MM_HeapRegionManagerTarok(env, regionSize, tableDescriptorSize,
...
MM_HeapRegionDescriptor*
MM_HeapRegionManagerTarok::internalAcquireSingleTableRegion(MM_EnvironmentBase* env,
                                                            MM_MemorySubSpace* subSpace,
...
void
MM_HeapRegionManagerTarok::internalLinkRegions(MM_EnvironmentBase* env,
                                                 ...
                                                 uintptr_t numaNode)
{
	...
	Trc_MM_HeapRegionManager_enableRegionsInTable_Exit(env->getLanguageVMThread(), regionCount, firstRegion,
	                                                   numaNode);
}
...
MM_HeapRegionManagerTarok::internalReleaseTableRegions(MM_EnvironmentBase* env, MM_HeapRegionDescriptor* rootRegion)
{
	...
	Assert_MM_true(rootRegion < (MM_HeapRegionDescriptor*)((uintptr_t)_regionTable
	                                                       + (_tableRegionCount * _tableDescriptorSize)));
...
MM_HeapRegionManagerTarok::getHeapMemorySnapshot(MM_GCExtensionsBase* extensions,
