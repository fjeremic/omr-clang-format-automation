...
MM_VerboseHandlerOutputStandard::enableVerbose()
{
	...
	(*_mmPrivateHooks)
	        ->J9HookRegisterWithCallSite(
	                _mmPrivateHooks, J9HOOK_MM_PRIVATE_ACQUIRED_EXCLUSIVE_TO_SATISFY_ALLOCATION,
...
MM_VerboseHandlerOutputStandard::disableVerbose()
{
	...
	/* GCLaunch */
	(*_mmPrivateHooks)
	        ->J9HookUnregister(_mmPrivateHooks, J9HOOK_MM_PRIVATE_SYSTEM_GC_START, verboseHandlerSystemGCStart,
	                           NULL);
	(*_mmPrivateHooks)
	...
	(*_mmPrivateHooks)
	        ->J9HookUnregister(_mmPrivateHooks, J9HOOK_MM_PRIVATE_EXCLUSIVE_ACCESS_RELEASE,
	...
	/* Concurrent GMP */
	(*_mmPrivateHooks)
	        ->J9HookUnregister(_mmPrivateHooks, J9HOOK_MM_PRIVATE_CONCURRENT_PHASE_START,
	                           verboseHandlerConcurrentStart, NULL);
	(*_mmPrivateHooks)
	        ->J9HookUnregister(_mmPrivateHooks, J9HOOK_MM_PRIVATE_CONCURRENT_PHASE_END, verboseHandlerConcurrentEnd,
	                           NULL);
	...
	(*_mmPrivateHooks)
	        ->J9HookUnregister(_mmPrivateHooks, J9HOOK_MM_PRIVATE_CONCURRENT_HALTED, verboseHandlerConcurrentHalted,
	                           NULL);
	(*_mmPrivateHooks)
	...
	/* Excessive GC */
	(*_mmOmrHooks)
	        ->J9HookUnregister(_mmOmrHooks, J9HOOK_MM_OMR_EXCESSIVEGC_RAISED, verboseHandlerExcessiveGCRaised,
	                           NULL);
}
...
void
MM_VerboseHandlerOutputStandard::handleGCOPOuterStanzaStart(MM_EnvironmentBase* env,
                                                            const char* type,
                                                            uintptr_t contextID,
                                                            uint64_t duration,
                                                            bool deltaTimeSuccess)
...
MM_VerboseHandlerOutputStandard::handleScavengeEndNoLock(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
	...
	if (event->cycleEnd) {
		writer->formatAndOutput(
		        env, 1, "<scavenger-info tenureage=\"%zu\" tenuremask=\"%4zx\" tiltratio=\"%zu\" />",
	...
	if (0 != scavengerStats->_tenureExpandedCount) {
		...
		outputCollectorHeapResizeInfo(env, 1, HEAP_EXPAND, scavengerStats->_tenureExpandedBytes,
		                              scavengerStats->_tenureExpandedCount, MEMORY_TYPE_OLD, SATISFY_COLLECTOR,
		                              expansionMicros);
	}
...
#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
void
MM_VerboseHandlerOutputStandard::handleConcurrentRememberedSetScanEnd(J9HookInterface** hook,
                                                                      uintptr_t eventNum,
                                                                      void* eventData)
{
	...
	writer->formatAndOutput(env, 1,
	                        "<scan objectsFound=\"%zu\" bytesTraced=\"%zu\" workStackOverflowCount=\"%zu\" />",
...
void
MM_VerboseHandlerOutputStandard::handleConcurrentCardCleaningEnd(J9HookInterface** hook,
                                                                 uintptr_t eventNum,
                                                                 void* eventData)
{
...
void
MM_VerboseHandlerOutputStandard::handleConcurrentCollectionStart(J9HookInterface** hook,
                                                                 uintptr_t eventNum,
                                                                 void* eventData)
{
...
void
MM_VerboseHandlerOutputStandard::handleConcurrentCollectionEnd(J9HookInterface** hook,
                                                               uintptr_t eventNum,
                                                               void* eventData)
{
                                               ...
                                               uintptr_t macroFragment)
{
	...
	bufPos += omrstr_printf(memInfoBuffer, INITIAL_BUFFER_SIZE,
	                        "<mem type=\"%s\" free=\"%zu\" total=\"%zu\" percent=\"%zu\"", type, (size_t)free,
	                        (size_t)total,
...
void
MM_VerboseHandlerOutputStandard::outputMemoryInfoInnerStanza(MM_EnvironmentBase* env,
                                                             uintptr_t indent,
                                                             MM_CollectionStatistics* statsBase)
{
	...
	if (stats->_loaEnabled) {
		...
		bufPos +=
		        omrstr_printf(tenureMemInfoBuffer, INITIAL_BUFFER_SIZE,
...
void
MM_VerboseHandlerOutputStandard::outputMemoryInfoInnerStanzaInternal(MM_EnvironmentBase* env,
                                                                     uintptr_t indent,
                                                                     MM_CollectionStatistics* statsBase)
{
	/* Empty stub */
