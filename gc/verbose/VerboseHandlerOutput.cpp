                                        ...
                                        uint64_t wallTimeMs)
{
	OMRPORT_ACCESS_FROM_OMRVM(_omrVM);
	uintptr_t bufPos = 0;
	bufPos += omrstr_printf(buf, bufsize, "id=\"%zu\" type=\"%s\" contextid=\"%zu\" timestamp=\"", id, type,
	                        contextId);
	bufPos += omrstr_ftime(buf + bufPos, bufsize - bufPos, VERBOSEGC_DATE_FORMAT_PRE_MS, wallTimeMs);
	bufPos += omrstr_printf(buf + bufPos, bufsize - bufPos, "%03llu", wallTimeMs % 1000);
	bufPos += omrstr_ftime(buf + bufPos, bufsize - bufPos, VERBOSEGC_DATE_FORMAT_POST_MS, wallTimeMs);
                                        ...
                                        uint64_t wallTimeMs)
{
	...
	bufPos += omrstr_printf(buf, bufsize,
	                        "id=\"%zu\" type=\"%s\" timems=\"%llu.%03.3llu\" contextid=\"%zu\" timestamp=\"", id,
	                        type, timeus / 1000, timeus % 1000, contextId);
	bufPos += omrstr_ftime(buf + bufPos, bufsize - bufPos, VERBOSEGC_DATE_FORMAT_PRE_MS, wallTimeMs);
	bufPos += omrstr_printf(buf + bufPos, bufsize - bufPos, "%03llu", wallTimeMs % 1000);
	bufPos += omrstr_ftime(buf + bufPos, bufsize - bufPos, VERBOSEGC_DATE_FORMAT_POST_MS, wallTimeMs);
	bufPos += omrstr_printf(buf + bufPos, bufsize - bufPos, "\"");
                                                    ...
                                                    uint64_t wallTimeMs)
{
	...
	bufPos += omrstr_printf(buf, bufsize,
	                        ...
	                        id, type, contextId, durationus / 1000, durationus % 1000, usertimeus / 1000,
	                        usertimeus % 1000, cputimeus / 1000, cputimeus % 1000);
	bufPos += omrstr_ftime(buf + bufPos, bufsize - bufPos, VERBOSEGC_DATE_FORMAT_PRE_MS, wallTimeMs);
	bufPos += omrstr_printf(buf + bufPos, bufsize - bufPos, "%03llu", wallTimeMs % 1000);
	bufPos += omrstr_ftime(buf + bufPos, bufsize - bufPos, VERBOSEGC_DATE_FORMAT_POST_MS, wallTimeMs);
	bufPos += omrstr_printf(buf + bufPos, bufsize - bufPos, "\"");
...
MM_VerboseHandlerOutput::handleInitializedRegion(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
	...
	writer->formatAndOutput(env, 2, "<attribute name=\"arrayletLeafSize\" value=\"%zu\" />",
...
MM_VerboseHandlerOutput::handleInitialized(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
	...
	writer->formatAndOutput(env, 1, "<attribute name=\"initialHeapSize\" value=\"0x%zx\" />",
	...
	writer->formatAndOutput(env, 1, "<attribute name=\"requestedPageSize\" value=\"0x%zx\" />",
	                        event->heapRequestedPageSize);
	writer->formatAndOutput(env, 1, "<attribute name=\"requestedPageType\" value=\"%s\" />",
	...
	writer->formatAndOutput(env, 1, "<attribute name=\"packetListSplit\" value=\"%zu\" />",
	                        _extensions->packetListSplit);
	writer->formatAndOutput(env, 1, "<attribute name=\"cacheListSplit\" value=\"%zu\" />",
...
MM_VerboseHandlerOutput::handleExclusiveStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
	...
	writer->formatAndOutput(env, 1,
	                        ...
	                        "lastid=\"%p\" lastname=\"%s\" />",
	                        exclusiveAccessTime / 1000, exclusiveAccessTime % 1000, meanIdleTime / 1000,
	                        meanIdleTime % 1000, event->haltedThreads,
...
void
MM_VerboseHandlerOutput::handleAllocationFailureStartInnerStanzas(J9HookInterface** hook,
                                                                  uintptr_t eventNum,
                                                                  void* eventData,
                                                                  uintptr_t indentDepth)
{}
...
MM_VerboseHandlerOutput::handleAllocationFailureStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
	...
	if (gc_policy_gencon == _extensions->configurationOptions._gcPolicy) {
		writer->formatAndOutput(env, 0,
		                        ...
		                        manager->getIdAndIncrement(), event->currentThread, event->requestedBytes,
		...
		writer->formatAndOutput(env, 0,
		                        ...
		                        manager->getIdAndIncrement(), event->currentThread, event->requestedBytes,
		                        tagTemplate, deltaTime / 1000, deltaTime % 1000, endOfTag);
	}
...
MM_VerboseHandlerOutput::handleFailedAllocationCompleted(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
	...
	if (TRUE == event->succeeded) {
		writer->formatAndOutput(env, 0,
		                        "<allocation-satisfied id=\"%zu\" threadId=\"%p\" bytesRequested=\"%zu\" />",
		                        id, currentThread->_language_vmthread, bytesRequested, tagTemplate);
	} else {
		writer->formatAndOutput(env, 0,
		                        "<allocation-unsatisfied id=\"%zu\" threadId=\"%p\" bytesRequested=\"%zu\" />",
		                        id, currentThread->_language_vmthread, bytesRequested, tagTemplate);
	}
	writer->flush(env);
	exitAtomicReportingBlock();
...
void
MM_VerboseHandlerOutput::handleAcquiredExclusiveToSatisfyAllocation(J9HookInterface** hook,
                                                                    uintptr_t eventNum,
                                                                    void* eventData)
{
...
MM_VerboseHandlerOutput::outputMemoryInfo(MM_EnvironmentBase* env, uintptr_t indent, MM_CollectionStatistics* stats)
{
	...
	if (hasOutputMemoryInfoInnerStanza()) {
		writer->formatAndOutput(
		        env, indent, "<mem-info id=\"%zu\" free=\"%zu\" total=\"%zu\" percent=\"%zu\">",
...
void
MM_VerboseHandlerOutput::outputMemoryInfoInnerStanza(MM_EnvironmentBase* env,
                                                     uintptr_t indent,
                                              ...
                                              uint64_t timeInMicroSeconds)
{
	...
	writer->formatAndOutput(env, indent,
	                        ...
	                        id, resizeTypeName, getSubSpaceType(subSpaceType), resizeAmount, resizeCount,
	                        timeInMicroSeconds / 1000, timeInMicroSeconds % 1000, reasonString, tagTemplate);
	writer->flush(env);
}

void
MM_VerboseHandlerOutput::outputCollectorHeapResizeInfo(MM_EnvironmentBase* env,
                                                       uintptr_t indent,
                                                       HeapResizeType resizeType,
                                                       uintptr_t resizeAmount,
                                                       uintptr_t resizeCount,
                                                  ...
                                                  uintptr_t cleared)
{
	...
	if (0 != candidates) {
		writer->formatAndOutput(env, ident, "<stringconstants candidates=\"%zu\" cleared=\"%zu\"  />",
		                        candidates, cleared);
	}
}
