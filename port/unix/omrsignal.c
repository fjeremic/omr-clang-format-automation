...
#define MSR_TS_MASK 0x6U
    ...
    if ((OMRPORT_SIG_EXCEPTION_COOPERATIVE_SHUTDOWN == result)
         ...
         *
         * 		Hardware signals - return from the signal handler. This is preferable as the signal we're handling is
         * reported in LE diagnostics as the cause of the crash. However, returning doesn't work for software signals.
         *
         * 		Software signals -  explicitly trigger an abend. Whereas this forces LE RRS in all cases,
         * 								LE diagnostics will report the JVM abend as the cause of the crash, which
         * somewhat clouds the issue.
         */
