                ...
                if (baseObject) {
                    ...
                    if (recognizedClassName) {
                        ...
                        if (recognizedClazz
                            ...
                            uintptrj_t fieldObject = vp->comp()->fej9()->getReferenceFieldAtAddress(
...
static TR::Node* constrainIfcmpeqne(OMR::ValuePropagation* vp, TR::Node* node, bool branchOnEqual)
{
    ...
    if (cannotBranch
        ...
        // lhsChild->getFirstChild()->getFirstChild() is not NULL, since non-NULL constraintFromMethodPointer implies
        // the following trees: n235n     ifacmpne --> block_25 BBStart at n230n (inlineProfiledGuard ) [0x7fffd5110a40]
        // bci=[0,0,314] rc=0 vc=56 vn=49 sti=- udi=- nc=2 flg=0x1020 n233n       iaload <vtable-entry-symbol> [#552
        // Shadow +768] [flags 0x10607 0x0 ]              [0x7fffd51109b0] bci=[-1,0,2302] rc=1 vc=56 vn=7 sti=- udi=-
        // nc=1 n232n iaload <vft-symbol> [#449 Shadow] [flags 0x18607 0x0 ] (X>=0 )                  [0x7fffd5110968]
        // bci=[-1,0,2302] rc=1 vc=56 vn=6 sti=- udi=- nc=1 flg=0x100 n3n             ==>aload n234n       aconst
        // 0x7fd070 (methodPointerConstant )                                          [0x7fffd51109f8] bci=[0,0,314]
        // rc=1 vc=56 vn=5 sti=- udi=- nc=0 flg=0x2000
        ...
        if (!disableProfiled2Overridden && callNode && vp->lastTimeThrough()) {
                    ...
                    == TR_yes) /*the object class may be less specific than callClass*/
            {
                ...
                if (methodSymbol
                    ...
                    // classes in the given hierarchy most likely rvm will also be NULL in this case, but it is better
                    // to not rely on this implicit connection between different CH queries.
