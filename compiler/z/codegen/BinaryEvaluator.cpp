        ...
        if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
            ...
            if (node->getOpCodeValue() == TR::lshl) {
                if (firstChild->getOpCodeValue() == TR::i2l && firstChild->isSingleRefUnevaluated()
                    ...
                    auto mnemonic = TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12) ? TR::InstOpCode::RISBGN
                                                                                             : TR::InstOpCode::RISBG;
        ...
        if (node->getOpCodeValue() == TR::lor) {
            ...
            if (lushr && lushr->getOpCodeValue() == TR::lushr && lushr->getReferenceCount() == 1
                ...
                if (rShftAmnt + lShftAmnt == 64 && lshlSourceNode == lushrSourceNode) {
                    ...
                    TR::InstOpCode::Mnemonic opCode = TR::Compiler->target.cpu.getSupportsArch(TR::CPU::zEC12)
    ...
    TR::Node* node, TR::CodeGenerator* cg, int32_t shiftAmount, bool isSignedShift)
{
    if (TR::Compiler->target.cpu.getSupportsArch(TR::CPU::z10)) {
        ...
        if (node->getOpCodeValue() == TR::land && secondChild->getOpCode().isLoadConst()) {
            ...
            // cannot perform a signed shift because we must preserve the sign bit. In general we cannot handle this
            // case because we are not certain our input will be non-negative, hence we must conservatively disallow the
            // optimization in such a case.
            if (longConstValue >= 0x8000000000000000LL && shiftAmount != 0 && isSignedShift) {
                return NULL;
            }
            // The Shift amount operand in RISBG only holds 6 bits so the optimization will not work on shift amounts
            // >63 or < 63.
            if (shiftAmount > 63 || shiftAmount < -63) {
                return NULL;
            }
            // If the node entered is a common node, then there is no guarantee that the child of the common node will
            // still be alive. And when we try to access the value in that node, we might receive values from a node
            // that might've been killed already. Thus, the optimization will not work.
            ...
            if (popCnt == (64 - lZeros - tZeros)) {
                ...
                if (firstChild->getReferenceCount() > 1 || lZeros > 31 || tZeros > 31 || (lZeros > 0 && tZeros > 0)) {
                    ...
                    // basically zeroing the register. This optimization will not be used. [2] Same case if the
                    // resultant lsb of a left shift is shifted out of range. Eg[1]: 000...0011 >> 3. msb = 62, lsb
                    // = 63. If we right shift, then msb and lsb will both be > 63 (out of range) Eg[2]: 11100...00
                    // << 10. msb = 0, lsb = 2. If we left shift, then msb and lsb will both be < 0 (out of range)
            ...
            if (doTransformation && performTransformation(comp, "O^O Use RISBG instead of 2 ANDs for %p.\n", node)) {
                ...
                // If the shift amount is zero, this instruction sets the rotation factor to 0 and sets the zero
                // bit(0x80). So it's effectively zeroing out every bit except the inclusive range of lsBit to msBit.
                // The bits in that range are preserved as is.
                //
                // If the shift amount is NOT zero, then we must account for the shifted range
                // [1] If the shifted lsb of a left shift is still in range, but the msb is out of range, then we can
                // still "capture" the shifted bits by upperbounding the msb. [2] Similar case if the msb of a right
                // shift is still in range but the lsb is out of range.
                //
                // Eg[1]: 00...0111 >> 2. msb = 61, lsb = 63. If we right shift, we get 000...0001, so we can still
                // preserve the last bit. Eg[2]: 1110...00 << 2. msb = 0, lsb = 2. If we left left, we can still
                ...
                generateRIEInstruction(
                    cg, opCode, node, targetReg, sourceReg, rangeStart, 0x80 + rangeEnd, shiftAmount);
                cg->decReferenceCount(firstChild);
                cg->decReferenceCount(secondChild);
                return targetReg;
            }
