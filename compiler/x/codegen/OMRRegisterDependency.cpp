...
uint32_t OMR::X86::RegisterDependencyConditions::numReferencedFPRegisters(TR::CodeGenerator* cg)
{
    ...
    for (i = 0; i < _numPreConditions; i++) {
        ...
        if ((reg && reg->getKind() == TR_X87)
            || (!reg
                && _preConditions->getRegisterDependency(i)->getRealRegister() == TR::RealRegister::AllFPRegisters)) {
            total++;
        }
    }
    ...
    for (i = 0; i < _numPostConditions; i++) {
        ...
        if ((reg && reg->getKind() == TR_X87)
            || (!reg
                && _postConditions->getRegisterDependency(i)->getRealRegister() == TR::RealRegister::AllFPRegisters)) {
            total++;
        }
    }
